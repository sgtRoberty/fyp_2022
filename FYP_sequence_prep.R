...
convt <- function(filename, format) {
  if (format == "protein"){
    seqtype <- "AA"
  }
  else if (format == "dna"){
...
convt("../data/5_aa_supermatrix.fasta", format = "protein")
...
##### Mitogenome metadata #####
AllMitoMetadata <- readr::read_csv("../data/AllMitogenomesMetadata.csv")
aa_supermatrix_metadata <- dplyr::filter(AllMitoMetadata, 
                                         db_id %in% names(aa_supermatrix_raw))
# 75 sequences in the supermatrix that don't have a match in AllMitoMetadata
length(names(aa_supermatrix_raw)[! names(aa_supermatrix_raw) %in% AllMitoMetadata$db_id])
table(aa_supermatrix_metadata$order)
non_coleoptera_metadata <- dplyr::filter(aa_supermatrix_metadata, 
                                         order != "Coleoptera")
non_coleoptera_metadata <- split(non_coleoptera_metadata, f = non_coleoptera_metadata$order)

table(aa_supermatrix_metadata$country)
Ecuador <- dplyr::filter(aa_supermatrix_metadata, country == "Ecuador")
Malaysia <- dplyr::filter(aa_supermatrix_metadata, country == "Malaysia")
Indonesia <- dplyr::filter(aa_supermatrix_metadata, country == "Indonesia")
Panama <- dplyr::filter(aa_supermatrix_metadata, country == "Panama")

unique(Malaysia$subregion)
unique(Malaysia$locality)
...
rapidnj_tree_noneg_rooted <- root(rapidnj_tree_noneg, outgroup = non_coleoptera_metadata$Megaloptera$db_id,
                                  resolve.root = T)
is.rooted(rapidnj_tree_noneg_rooted)
rapidnj_tree_noneg_rooted_plot <- ggtree(rapidnj_tree_noneg_rooted, 
                                         branch.length = 'none',
                                         layout="circular") %<+% 
  AllMitoMetadata + geom_tree(aes(color=superfamily)) + geom_rootedge(rootedge = 20) + 
  geom_tippoint(aes(color=superfamily), alpha=0.45, size=0.4) + 
...
rapidnj_tree_noneg_rooted_Malaysia <- keep.tip(rapidnj_tree_noneg_rooted, Malaysia_tips)
rapidnj_tree_noneg_rooted_Panama <- keep.tip(rapidnj_tree_noneg_rooted, Panama_tips)
...
mb_sampletrees_plot <- ggtree(mb_sampletrees[[length(mb_sampletrees)]], branch.length='none', 
  ...
  theme(legend.position="right")
ggsave("../results/1000subset_1000mtaa_test.run1_superfamilytree.pdf", plot = mb_sampletrees_plot, 
       width = 20, height = 20)
