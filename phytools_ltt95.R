...
plot.ltt95<-function(x,...){
  ...
  if(attr(x,"method")=="times"){
    ...
    plot(x[,3],x[,1],lwd=2,xlim=x.lim, cex.axis=1.3, 
         type=if(attr(x,"mode")=="median") "s" else "l",main=NULL,
         xlab=if(xaxis=="standard") "Time from the oldest root (Millions of years)" else 
           if(xaxis=="negative") "" else 
             if(xaxis=="flipped") "Time before the present day (Millions of years)",
         ylab="",log=if(log) "y" else "")
    if(!shaded){
    ...
    plot(x[,1],x[,3],xlim=x.lim,ylim=c(min(x[,2]),max(x[,4])),lwd=2, cex.axis=1.3, 
         type=if(attr(x,"mode")=="median") "s" else "l",main=NULL,
         xlab=if(xaxis=="standard") "Time from the oldest root (Millions of years)" else 
           if(xaxis=="negative") "" else 
             if(xaxis=="flipped") "Time before the present day (Millions of years)",
